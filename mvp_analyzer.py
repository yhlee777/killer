...
# mvp_analyzer.py의 is_owner_reply 함수를 이것으로 교체!
...
def is_owner_reply(review_text):
    ...
    medium_count = sum(1 for signal in medium_signals if signal in review_text)
    if medium_count >= 2:
        return True
    ...
    for pattern in honorific_patterns:
        if re.search(pattern, review_text):
            # 존댓말이 있고 + 감사/사과 단어가 있으면 사장님 답변
            if any(word in review_text for word in ['감사', '사과', '죄송', '개선']):
                return True
    
    # 4. 길이 기반 필터 (너무 짧으면서 감사 표현이 있으면 사장님 답변)
    if len(review_text) < 50:  # 50자 미만
        if any(word in review_text for word in ['감사드립니다', '감사합니다', '죄송합니다']):
            return True
...
if __name__ == "__main__":
    ...
    test_cases = [
        # 사장님 답변 (True 반환해야 함)
        ("식사 중 직원의 부적절한 언행으로 인해 기념일 자리가 불편한 시간이 되셨다는 점에 대해 깊이 사과드립니다.", True),
        ("맛에 아쉬움이 있으셨다니 정말 죄송합니다.", True),
        ("소중한 리뷰 감사드립니다. 더욱 노력하겠습니다.", True),
        ("방문해주셔서 감사합니다!", True),
        ("귀한 시간 내어 방문해주셔서 감사드립니다.", True),
        ("불편을 드려 죄송합니다. 개선하겠습니다.", True),
        
        # 실제 고객 리뷰 (False 반환해야 함)
        ("남자친구도 너무 맛 없어졌다고 대실망이래요", False),
        ("예약이 무색하게 어정쩡한 구석자리를 안내받았는데 테이블이 끈적끈적..", False),
        ("직원 - 응대 태도 불친절함. 동네 중국집 직원들이 더 친절할 수준", False),
        ("날씨가 한여름인데 에어컨 안틀어준건 좀실망이네요", False),
        ("콜라가 6000원인건 좀 아까웠지만", False),
        ("음식은 맛있었는데 서비스가 아쉬웠어요", False),
    ]
